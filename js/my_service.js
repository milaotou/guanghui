//更多服务
//Tab
function resetTabs(obj) {
	$(obj).parent().parent().next("div").find("div").hide();
	$(obj).parent().parent().find("a").removeClass("current");
}
function loadTab() {
	$(".content > div").hide();
	$(".tabs").each(function () {
		$(this).find("li:first a").addClass("current");
	});
	$(".content").each(function () {
		$(this).find("div:first").fadeIn();
		$(this).find('.date_text').addClass('xian');
		$(this).find('.index-nf').addClass('xian');
		$(this).find('.index-yf').addClass('xian');
		$(this).find('.index-rz').addClass('xian');
		$(this).find('.rq-cycle').addClass('xian');
        $(this).find('.diy_select,.diy_select_txt,.diy_select_btn').addClass('xian');
	});
	$(".tabs a").on("click", function (e) {
		e.preventDefault();
		if ($(this).attr("class") == "current") {
			return;
		} else {
			resetTabs(this);
			$(this).addClass("current");
			$($(this).attr("name")).fadeIn();
		}
	});
}


$(function() {
	//日期
	$('.date_slt').click(function() {
		var $rq = $('.rq').toggle();
		if ($rq.css('display') == 'none') {
			return false;
		}
		var $year = $('.year_cycle');
		var $month = $('.month_cycle');
		var $ri = $('.date_cycle');
		if (!$rq.data('init')) {
			$rq.data('init', true);
			$('.year_prev').click(function() {
				$year.data('cycle.opts').fx = 'scrollDown';
			})
			$('.year_next').click(function() {
				$year.data('cycle.opts').fx = 'scrollUp';
			})
			$year.cycle({
				fx: 'scrollVert',
				slideResize: 0,
				speed: 500,
				timeout: 0,
				prev: '.year_prev',
				next: '.year_next',
				after: function(currSlideElement, nextSlideElement, options, forwardFlag) {
					$('.year_prev').css('visibility', options.currSlide < 1 ? 'hidden' : 'visible' );
					$('.year_next').css('visibility', options.currSlide+1 >= options.slideCount ? 'hidden' : 'visible' );
				}
			});
			$('.month_prev').click(function() {
				$month.data('cycle.opts').fx = 'scrollDown';
			})
			$('.month_next').click(function() {
				$month.data('cycle.opts').fx = 'scrollUp';
			})
			$month.cycle({
				fx: 'scrollUp',
				slideResize: 0,
				speed: 500,
				timeout: 0,
				prev: '.month_prev',
				next: '.month_next',
				after: function(currSlideElement, nextSlideElement, options, forwardFlag) {
					$('.month_prev').css('visibility', options.currSlide < 1 ? 'hidden' : 'visible' );
					$('.month_next').css('visibility', options.currSlide+1 >= options.slideCount ? 'hidden' : 'visible' );
				}
			});

			$('.date_prev').click(function() {
				$ri.data('cycle.opts').fx = 'scrollDown';
			})
			$('.date_next').click(function() {
				$ri.data('cycle.opts').fx = 'scrollUp';
			})
			$ri.cycle({
				fx: 'scrollUp',
				slideResize: 0,
				speed: 500,
				timeout: 0,
				prev: '.date_prev',
				next: '.date_next',
				after: function(currSlideElement, nextSlideElement, options, forwardFlag) {
					$('.date_prev').css('visibility', options.currSlide < 1 ? 'hidden' : 'visible' );
					$('.date_next').css('visibility', options.currSlide+1 >= options.slideCount ? 'hidden' : 'visible' );
				}
			});
		}
		// var date = $('#date_text').text().split('/');
		// var year = date[0];
		// var month = date[1];

		var index = $year.find('li.Choice').parent().data('index');
		index !== undefined && $year.cycle(index);

		var index = $month.find('li.Choice').parent().data('index');
		index !== undefined && $month.cycle(index);

		var index = $ri.find('li.Choice').parent().data('index');
		index !== undefined && $ri.cycle(index);

		return false;
	});

	$('.rq').on('click', '.year_cycle li', function() {
		$('.year_cycle').find('li').removeClass('Choice');
		$(this).addClass('Choice');
	}).on('click', '.month_cycle li', function() {
		$('.month_cycle').find('li').removeClass('Choice');
		$(this).addClass('Choice');
	}).on('click', '.date_cycle li', function() {
		$('.date_cycle').find('li').removeClass('Choice');
		$(this).addClass('Choice');
	}).on('click', '.rq-cycle li', function() {
		var year = $('.year_cycle').find('.Choice').attr('data');
		var month = $('.month_cycle').find('.Choice').attr('data');
		var ri = $('.date_cycle').find('.Choice').attr('data');
		$('.date_text').text( year + '年' + month+'月'+ri+'日' );
		$('#form').find('input[name=car_buy_date]').val( year + month );
	}).click(function() {
		return false;
	})

	$(document).on('click', function() {
		$('.rq').hide();
	});

	$(".hea_nav li").hover(function() {
		$(this).css('border-top', '3px #ff6c00 solid');
		$(this).find('a').addClass('nav_mo');
	}, function() {
		$(this).css('border-top', '3px #1e1e1e solid');
		$(this).find('a').removeClass('nav_mo');
	});
	$(".hea_nav ul li:last").css('margin-right', '0');
    
});
$(function(){
	$('.month_cycle>ul>li').eq(1).click(function(){
		var a=$(this).parents('.index-yf').siblings('.index-nf').find(".year_cycle>ul>li.Choice").text();
		if ((a%4==0&&a%100!=0 )||a%400==0) {
			 $(".runnian>li").css("display","none");
			 $(".runnian>li:first").css("display","block");
		}else{
             $(".runnian>li").css("display","none");
		}		
	});
	$(".month_cycle>ul .sanyi").click(function(){
		$(".runnian>li").css("display","block");

	});
	$(".month_cycle>ul .sanshi").click(function(){
		$(".runnian>li").css("display","block");
	    $(".runnian>li").eq(2).css("display","none");
	});
})
